...
**Proyecto:** Products Builder  
**Versión:** 0.1.0  
**Repo:** https://github.com/jsotoexelixitech/products-builder  
**Audiencia:** Desarrollador backend  

Este documento describe **exactamente** qué envía el frontend y qué debe responder el backend. Si se implementa tal cual, la integración funciona sin cambios en el UI.
...
2. [Enums y constantes](#2-enums-y-constantes)
3. [Mapa wizard → endpoints](#3-mapa-wizard--endpoints)
4. [Productos](#4-productos)
5. [Coberturas](#5-coberturas)
6. [Planes comerciales](#6-planes-comerciales)
7. [Actuarial](#7-actuarial)
8. [Legal](#8-legal)
9. [Flujo de emisión](#9-flujo-de-emisión)
10. [Workflow SUDEASEG](#10-workflow-sudeaseg)
11. [SISIP (opcional)](#11-sisip-opcional)
12. [Objeto Product completo](#12-objeto-product-completo)
13. [Errores](#13-errores)
14. [Reglas de negocio transversales](#14-reglas-de-negocio-transversales)
15. [Referencias en el código](#15-referencias-en-el-código)
...
| 6 | Revisión SUDEASEG | `GET .../validate-submission` + `POST .../transition` | `GET /products/:id` |
...
**Response:** `200` — objeto `Product` completo (ver [sección 12](#12-objeto-product-completo)).

**Errores:** `404` si no existe.

---

### 4.3 Crear producto (wizard paso 0 — primera vez)
...
### 4.4 Actualizar producto (wizard paso 0 — edición)
...
### 5.1 Reemplazar todas las coberturas (wizard paso 1)
...
**Campos que el frontend NO envía hoy** (pero el DTO backend los acepta):

- `insuredSumMin`, `insuredSumMax` (legacy; el UI usa solo `insuredSumFixed`)
...
### 5.2 Listar / crear / eliminar cobertura individual
...
### 6.1 Obtener planes (wizard paso 2 — al entrar)
...
### 6.2 Guardar planes (wizard paso 2)
...
### 7.1 Guardar actuarial (wizard paso 3)
...
### 8.1 Guardar bundle legal (wizard paso 4)
...
### 9.1 Obtener config (wizard paso 5 — al entrar)
...
### 9.2 Guardar config (wizard paso 5)
...
## 10. Workflow SUDEASEG

### 10.1 Validar envío (wizard paso 6 — al guardar)
...
- [ ] `PUT /coverages` devuelve array con `id`
- [ ] `GET/PUT /plans` valida `coverageIds`
- [ ] `PUT /actuarial` calcula `commercialPremium`
...
*Documento de contrato API v0.1.0 — Products Builder / Exelixi Tech*
